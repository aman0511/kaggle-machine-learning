...
df_submit = pd.read_csv('porto-sample_submission.csv')
...
# Train model
model.fit(x_train, y_train, batch_size=1024, epochs=1, validation_data=(x_valid, y_valid), verbose=1, callbacks=[callback_early_stopping])
...
df_submit.to_csv('porto-submission-keras.csv', index=False)
